...
start_pipeline (WebRTC * webrtc)
{
  ...
  webrtc->pipe =
          ...
          //                   "videotestsrc is-live=true pattern=ball ! video/x-raw,width=[320,1280] ! queue max-size-buffers=1 ! videoconvert ! "
  ...
  g_print ("GOOGLIEBAH: A\n");
  if (error) {
    g_printerr ("Failed to parse launch: %s\n", error->message);
    g_print ("GOOGLIEBAH: Error: %s\n", error->message);
    g_error_free (error);
    goto err;
  }
  g_print ("GOOGLIEBAH: B\n");
                   ...
                   GBytes * message, WebRTC * webrtc)
{
  ...
  if (g_strcmp0 (text, "HELLO") == 0) {
    ...
    if (!start_pipeline (webrtc))
      cleanup_and_quit_loop (webrtc, "ERROR: failed to start pipeline2",
                             PEER_CALL_ERROR);
    /* Handle errors */
